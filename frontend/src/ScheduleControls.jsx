...
import { CalendarDays, ChevronLeft, ChevronRight, ClipboardList, Download, Gauge, List, Map as MapIcon, MapPin, Moon, Search, Shield, SlidersHorizontal, X } from "lucide-react";
import Metric from "./Metric.jsx";
import TrackMap from "./TrackMap.jsx";
import { ACTIVITY_PRIORITIES, activityPriority, calendarDays, isoDate, locationOptions, matches, occupancyOverlay } from "./schedule-utils.js";
...
/** Line → directional track → location options, plus the current value when it is a station picked on the map. */
function LocationOptions({ tasks, value, verbose = false }) {
  const { locations, lines, tracks } = locationOptions(tasks);
  const known = locations.includes(value) || lines.includes(value) || tracks.includes(value);
  return <>
    <option value="">All locations</option>
    {lines.map((line) => <option key={line} value={line}>Line {line}{verbose ? " · all tracks" : ""}</option>)}
    {tracks.map((track) => <option key={track} value={track}>{track.replace(":", " · ")}{verbose ? " track" : ""}</option>)}
...
export function SearchDialog({ tasks, query, priority, location, onApply, onClose, onMap }) {
  ...
  return <Dialog title="Search schedule" subtitle="Find a project and narrow its scheduled possessions." onClose={onClose}>
    <form onSubmit={(e) => { e.preventDefault(); onApply({ ...draft, query: draft.query.trim() }); }} className="p-5 space-y-5">
      ...
      <div><label htmlFor="search-location" className="mb-2 block text-xs text-slate-300">Filter by location</label><select id="search-location" className={input} value={draft.location} onChange={(e) => setDraft({ ...draft, location: e.target.value })}><LocationOptions tasks={tasks} value={draft.location} verbose /></select></div></div>
...
/** `downloads` — optional [{ name, href }] for the solver output CSVs; rendered next to the objective metrics. */
export function KdaDialog({ data, downloads = [], onClose }) {
  ...
  return <Dialog title="Schedule Score" subtitle={`${scenario.name} · ${scenario.label}`} icon={Gauge} onClose={onClose} wide>
    <div className="p-5 space-y-5">
      ...
      {downloads.length > 0 && (
        <div className="flex flex-wrap items-center gap-2 rounded-xl bg-slate-950/40 ring-1 ring-slate-800 px-4 py-3">
          <span className="text-xs text-slate-400 mr-auto">Export this run · solver output CSVs</span>
          {downloads.map(({ name, href }) => (
            <a key={name} href={href} className={`${button} !ring-slate-800`}>
              <Download className="h-3.5 w-3.5 text-sky-300" /> {name.replace(".csv", "")}
            </a>
          ))}
        </div>
      )}
...
/**
 * Location picker on the four-track schematic. Occupancy and buffer-exclusion
 * overlays come from the user's scoped tasks, optionally narrowed to one week.
 */
export function LocationMap({ network, tasks, location, week: initialWeek = null, onSelect, onClose }) {
  const [week, setWeek] = useState(initialWeek ?? "");
  const weeks = [...new Set(tasks.map((t) => t.week))].sort((a, b) => a - b);
  const scoped = week === "" ? tasks : tasks.filter((t) => t.week === Number(week));
  const { occupied, buffer } = occupancyOverlay(scoped);

  return (
    <Dialog title="Choose a location" subtitle="Click a sector, platform, station or directional track to filter the schedule." icon={MapIcon} onClose={onClose} wide>
      <div className="p-5 space-y-5">
        <div className="flex flex-wrap items-center gap-3">
          <p className="text-[11px] text-slate-500 mr-auto">Network schematic · not to scale · four directional tracks (EB / WB). Station filters include both platforms and adjacent sectors.</p>
          <label className="inline-flex items-center gap-2 text-xs text-slate-400">
            Overlay week
            <select aria-label="Overlay week" className={`${input} !w-auto !py-1.5 !text-xs`} value={week} onChange={(e) => setWeek(e.target.value)}>
              <option value="">All weeks</option>
              {weeks.map((w) => <option key={w} value={w}>Week {w}</option>)}
            </select>
          </label>
        </div>
        <TrackMap network={network} occupied={occupied} buffer={buffer} selected={location} onSelect={onSelect}
          caption={`${scoped.length} access night${scoped.length === 1 ? "" : "s"} in scope`} />
    ...
    <div className="space-y-3 mb-3">
      <div className="flex flex-wrap justify-end gap-2">
        <button disabled={disabled} onClick={onKda} className={`${button} !ring-sky-500/30 !text-sky-300 !bg-sky-500/5 disabled:opacity-40`}>
          <Gauge className="h-4 w-4" /> Schedule Score
        </button>
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-xl bg-slate-900/60 ring-1 ring-slate-800 px-3 py-3">
        ...
        <div className="relative flex-1 min-w-[170px] max-w-[310px]">
          ...
          <select aria-label="Filter by location" className={`${input} !pl-8 !py-2 !text-xs`}
            ...
            <LocationOptions tasks={tasks} value={filters.location} />
          </select>
        </div>
